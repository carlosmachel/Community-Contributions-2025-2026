...
Estado Experimental: Ressaltamos que tanto o A2A quanto o MCP ainda estão em estágio experimental, com documentações e diagramas de arquitetura em fase de maturação.
Segurança e Riscos: Discutimos os grandes desafios técnicos, especialmente a vulnerabilidade a prompt injection. Alertamos que agentes sem filtros ou permissionamento adequado podem ser induzidos a executar comandos maliciosos, como instruções SQL para apagar bancos de dados (drop database).
Evolução das Plataformas: Mencionamos como o Microsoft AI Foundry está se tornando um "guarda-chuva" para agrupar diversos serviços de IA generativa.
Aplicações Práticas: Citamos que o uso de agentes para apoio em estruturas de código e arquitetura de negócios já apresenta resultados interessantes, sendo considerado um caminho sem volta para o setor.</td>
...
No vídeo falamos também sobre dicas básicas de engenharia de prompt, incentivando os usuários a escrever perguntas mais claras, com contexto e instruções específicas, para extrair respostas mais precisas, relevantes e úteis da IA.</td>
...
A ausência de autenticação nativa em muitos servidores exige proteções externas, como o uso de API Management ou Microsoft Entra ID. Além disso, a observabilidade por meio de OpenTelemetry é fundamental para rastrear as ações do agente e detectar comportamentos maliciosos em tempo real. Por fim, destaca-se a necessidade de cautela com a "supply chain" e a realização de testes em ambientes controlados antes da produção.
  ...
  <tr>
      ...
      <td align=center>
      O vídeo aborda os riscos de segurança e as melhores práticas ao utilizar servidores Model Context Protocol (MCP) em agentes de IA., Entre os principais ataques discutidos estão o "poison tool description", onde descrições maliciosas enganam a IA para executar ações indevidas (como vazar conversas privadas), e a "colisão de nomes", que utiliza nomes similares a ferramentas legítimas para induzir o sistema ao erro.,, Outro perigo destacado é o ataque "cross-connector", no qual um comando oculto em um documento aciona ações maliciosas através de outro conector.,
Para mitigar essas ameaças, os especialistas recomendam o princípio do menor privilégio, limitando as permissões das ferramentas ao estritamente necessário. Sugere-se priorizar fontes verificadas, como o catálogo da Docker, ou desenvolver MCPs próprios para manter controle total sobre dados e credenciais. Por fim, o uso de filtros automatizados, sanitização de templates e ferramentas de observabilidade (tracing) são essenciais para monitorar interações suspeitas.
</td>
...
Na prática, o tutorial demonstra a criação de uma aplicação console, detalhando a configuração de variáveis de ambiente e o uso de pacotes NuGet para instanciar agentes. O exemplo ilustra como implementar respostas diretas ou via streaming, servindo de base para futuras explorações sobre conceitos avançados e prontidão para produção</td>
...
O conteúdo explica que, embora a implementação interna varie entre diferentes provedores, o framework oferece uma estrutura global para facilitar o desenvolvimento de assistentes que mantêm a continuidade e a coerência do diálogo.
</td>
  ...
  <tr>
      ...
      <td align=center>
      O vídeo aborda a lista OWASP Top 10 voltada especificamente para software open source, destacando os riscos de segurança em dependências e pacotes como npm, NuGet, Maven e imagens Docker. Os palestrantes discutem o primeiro item da lista, "Vulnerabilidades Conhecidas", ressaltando a importância de monitorar CVEs e utilizar bancos de dados como o do NIST e o GitHub Security Advisories.
Um ponto crítico detalhado é o conceito de dependências transitivas, exemplificado pelo impacto global da vulnerabilidade no Log4j, que afetou a integridade e disponibilidade de sistemas em larga escala. Além disso, o vídeo alerta sobre o risco de "Name Confusion" (confusão de nomes), onde pacotes maliciosos imitam nomes legítimos para enganar desenvolvedores.
Para mitigação, recomenda-se o uso do GitHub Dependabot, que automatiza alertas de segurança. Por fim, são apresentadas ferramentas de análise como Snyk, Checkmarx e opções open source como OWASP Dependency Check e Docker Scout.
</td>
  ...
  <tr>
      ...
      <td align=center>
      O vídeo aborda os riscos listados no OWASP Top 10 para Software de Código Aberto (OSS), com foco especial no comprometimento de pacotes legítimos. Os especialistas discutem como atacantes injetam código malicioso em projetos confiáveis através de hijacking de contas de mantenedores ou vulnerabilidades em repositórios, afetando a integridade de sistemas de build e dados.
São analisados casos reais de ataques à cadeia de suprimentos, como o da SolarWinds e o uso indevido de pipelines para mineração de Bitcoin. O debate também detalha vulnerabilidades de injeção (SQL, comandos de sistema e APIs), ressaltando que o ecossistema npm é particularmente visado devido a práticas de atualização automática de versões.
Como medidas de mitigação, o vídeo recomenda o uso de repositórios internos seguros (como Nexus ou Harbor) para cache de dependências, revisões de código e a adoção de frameworks de segurança para verificar a proveniência dos componentes.
</td>
...
A demonstração será totalmente implementada em C# (.NET), mostrando: como projetar e implantar agentes de IA no Microsoft Foundry; como os agentes chamam endpoints do Logic Apps para executar processos de negócios; como lidar com respostas, contexto e orquestração no .NET; e as melhores práticas para conectar IA e automação em ambientes de produção.
</td>
...
A live também aborda temas críticos como segurança em IA, destacando vulnerabilidades em modelos de linguagem (LLMs) e riscos como o tool poisoning. Por fim, enfatiza a importância da padronização e governança das tecnologias de IA.
A conclusão geral é que 2026 será um ano de amadurecimento, com foco em integração, segurança e consolidação das ferramentas de IA no mercado corporativo.
</td>
...
Como tendências-chave, destacam-se a arquitetura orientada a eventos, que amplia a conectividade com IA, e o papel do arquiteto de nuvem, cada vez mais focado em governança e resolução de problemas de negócio. Por fim, o conceito de idempotência é ressaltado como essencial para integrações seguras com agentes de IA.
</td>
...
Machel demonstra na prática como migrar o código existente, destacando as diferenças nas chamadas de API e nas estruturas de objetos. Um ponto importante é que a Response API pode ser usada independentemente da Conversation API para gerenciar o estado da conversa.
As principais vantagens da nova abordagem incluem fluxos de trabalho simplificados, armazenamento de conversas mais robusto e maior flexibilidade no desenvolvimento de agentes com IA no Azure.
</td>
...
Na demonstração prática com uma minimal API, o presenter mostra a diferença entre perguntas simples (capital do Brasil, sem aprovação) e consultas de temperatura, que acionam o fluxo de aprovação. O debugging confirma que a função só é executada após consentimento explícito do usuário, garantindo controle, segurança e prevenção de ações indesejadas.
      </td>
...
O vídeo também explora o tratamento de erros ao associar ferramentas a conversas com agentes já definidos, além de demonstrar chamadas diretas ao modelo de resposta sem um agente, mantendo o histórico de conversa.
As principais tecnologias abordadas incluem Azure AI Foundry, Vector Store, File Search Tool, Persistent Agents e a API de Responses, destacando como a nova plataforma abstrai a complexidade do processamento e chunking de arquivos.
      </td>
...
O grupo apresenta o Spec Kit, ferramenta para desenvolvimento orientado a especificações, e discute como empresas podem customizar modelos via Azure AI Foundry, integrando bases internas e criando agentes especializados. A mensagem central é clara: IA é um aliado poderoso, mas o pensamento crítico do desenvolvedor continua indispensável.
      </td>
...
O framework se destaca pela flexibilidade, possibilitando adicionar ou modificar middlewares em execuções específicas sem alterações permanentes. O uso de middlewares é essencial para aumentar a segurança, o controle e a observabilidade em sistemas baseados em agentes de inteligência artificial.
</td>
...
O código está disponível no repositório GitHub da série, e a implementação utiliza recursos como Chat Message, Data Content e o método Run Async com um DTO específico para estruturar automaticamente as respostas.
O vídeo também aborda o uso de streaming para saídas estruturadas, onde atualizações são recebidas continuamente e consolidadas em um único DTO. A conclusão reforça os benefícios do framework na abstração de funcionalidades complexas ao trabalhar com modelos de IA.
</td>
...
O vídeo explora ferramentas de debugging e observabilidade, permitindo rastrear decisões dos agentes e monitorar consumo de tokens. Também são demonstrados workflows low-code integrados ao Logic Apps e implementações práticas com OpenTelemetry, Grafana e Elastic para telemetria e monitoramento. O conteúdo é voltado para desenvolvedores que buscam soluções corporativas robustas com IA.</td>
